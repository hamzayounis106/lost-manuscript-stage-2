...
function SinglePlace() {
  ...
  const { city_name, contributionDate, authors } = place;
  ...
  const authorRecords = authors.map((author) =>
    data.records.find((record) => record.person_name === author)
  );
          ...
          <div className="p-8 mt-4 rounded bg-white flex-1 border border-[#CCCCCC]">
            <div>
              ...
              <ul className="pl-12 list-disc">
                {authorRecords?.map((dt, idx) => (
                  <li key={idx} className="text-[14.5px]">
                    <Link to={`/catalogue/${dt.book_title}`}>
                      {dt.person_name}
                    </Link>
                  </li>
                ))}
            ...
            <div>
              ...
              <ul className="pl-12 list-disc">
                <li className="text-[14.5px]">{contributionDate}</li>
              </ul>
            </div>
            <div>
